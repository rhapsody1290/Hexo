...
1、jstat -gcutil 发现存活区占用100%，根据书上的经验公式，将堆的大小设置为4倍的 -Xmn Xms
...
linux常用命令，linux统计一个文本中出现的单词数量
	...
	more file | grep -o word | wc -l #wc -l ：统计行数
...
ACID:原子性（操作要么全部成功，要不失败回滚）、一致性（转账钱没少）、隔离性（并发访问数据库时，是事务间不受影响）、持久性（保存到磁盘）
